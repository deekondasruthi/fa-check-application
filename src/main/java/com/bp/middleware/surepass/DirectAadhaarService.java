...
@Service
public class DirectAadhaarService {
	...
	public ResponseStructure directAadhaar(RequestModel model, HttpServletRequest servletRequest) {
			...
			if(!verificationModel.isStatus() || !vendorModel.isStatus()) {
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return aadhaarReplica.aadhaarDirectDuplicate(model, userModel,verificationModel);
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return surepassDirectAadhaar(model, userModel, verificationModel, vendorModel, merchantPriceModel,
	...
	private ResponseStructure surepassDirectAadhaar(RequestModel model, EntityModel userModel,
