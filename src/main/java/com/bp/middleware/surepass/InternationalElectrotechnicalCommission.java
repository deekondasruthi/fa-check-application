...
import com.bp.middleware.duplicateverificationresponse.InternationalElectrotechnicalCommissionReplica;
...
@Service
public class InternationalElectrotechnicalCommission {

	
	private final String VERIFICATION_TYPE = AppConstants.IEC;
	...
	@Autowired
	private InternationalElectrotechnicalCommissionReplica internationalElectrotechnicalCommissionReplica;

	public ResponseStructure internationalElectroTechCommission(RequestModel model, HttpServletRequest servletRequest) {
		...
		try {
			...
			if (userModel != null  && userModel.isAccountStatus()) {
				...
				VendorVerificationModel verificationModel = verificationRepository
						.findByVerificationDocument(AppConstants.IEC);
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey) && userModel.getNoRestriction() == 0) {
						...
						return internationalElectrotechnicalCommissionReplica.IecResponse(model, userModel,
								verificationModel);
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return surepassIec(model, userModel, verificationModel, vendorModel, merchantPriceModel,
	...
	private ResponseStructure surepassIec(RequestModel model, EntityModel userModel,
		...
		if (status) {
			...
			JSONObject details = internalData.getJSONObject("personal_details");
		...
		JSONObject commonResponse = CommonResponseStructure.commonResponseIec(temporary);
