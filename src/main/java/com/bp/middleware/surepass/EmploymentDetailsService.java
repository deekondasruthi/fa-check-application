...
import com.bp.middleware.duplicateverificationresponse.EmploymentDetailsReplica;
...
public class EmploymentDetailsService {
	...
	private final String VERIFICATION_TYPE = AppConstants.EMPLOYMENT_DETAILS;
	...
	@Autowired
	private EmploymentDetailsReplica employmentDetailsReplica;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;
	
	
	public ResponseStructure employmentDetails(RequestModel model, HttpServletRequest servletRequest) {
		...
		try {
			...
			if (userModel != null  && userModel.isAccountStatus()) {
				...
				VendorVerificationModel verificationModel = verificationRepository
						.findByVerificationDocument(AppConstants.EMPLOYMENT_DETAILS);
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return employmentDetailsReplica.EmploymentDetailsResponse(model, userModel,verificationModel);
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return surepassEmployment(model, userModel, verificationModel, vendorModel,
	...
	private ResponseStructure surepassEmployment(RequestModel model, EntityModel userModel,
		...
		if (status) {
			...
			JSONArray employmentHistory = internalData.getJSONArray("employment_history");
			...
			temporary.setEmploymentHistory(employmentHistory);
		...
		JSONObject commonResponse = CommonResponseStructure.commonResponseEmploymentHistory(temporary);
