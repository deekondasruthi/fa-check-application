...
import com.bp.middleware.duplicateverificationresponse.FssaiReplica;
...
public class FssaiService {
	...
	private final String VERIFICATION_TYPE = AppConstants.FSSAI;
	...
	@Autowired
	private FssaiReplica fssaiReplica;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;

	public ResponseStructure foodSafetyAndStandards(RequestModel model, HttpServletRequest servletRequest) {
			...
			if (userModel != null  && userModel.isAccountStatus()) {
				...
				if (merchantPriceModel != null && vendorPriceModel!=null  && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return fssaiReplica.fssaiDuplicateResponse(model, userModel,verificationModel);
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return surepassFssai(model, userModel, verificationModel, vendorModel, merchantPriceModel,
	...
	private ResponseStructure surepassFssai(RequestModel model, EntityModel userModel,
		...
		if (status) {
			...
			String fssaiNumber = internalData.optString("fssai_number");
			String applicationNumber = internalData.optString("application_number");
			
			JSONArray details = internalData.getJSONArray("details");
			JSONObject metaDetails = details.getJSONObject(0);
			
			String addressPresis = metaDetails.optString("address_premises");
			String statusDesc = metaDetails.optString("status_desc");
			String distrctName = metaDetails.optString("district_name");
			String displayRefId = metaDetails.optString("display_ref_id");
			String talukName = metaDetails.optString("taluk_name");
			String companyName = metaDetails.optString("company_name");
			String appTypeDesc = metaDetails.optString("app_type_desc");
			String stateName = metaDetails.optString("state_name");
			String licenseCategoryName = metaDetails.optString("license_category_name");
			String appSubmitionDate = metaDetails.optString("app_submission_date");
			String lastUpdatedOn = metaDetails.optString("last_updated_on");
			int fboId = metaDetails.optInt("fbo_id");
			int refId = metaDetails.optInt("ref_id");
			
			String statePremesis = metaDetails.optString("state_premises");
			int districtPremesis = metaDetails.optInt("district_premises");
			int talikPremesis = metaDetails.optInt("taluk_premises");
			String appType = metaDetails.optString("app_type");
			int pincode = metaDetails.optInt("pincode");
			...
			temporary.setFssaiNumber(fssaiNumber);
			temporary.setApplicationNumber(applicationNumber);
			temporary.setAddressPresis(addressPresis);
			temporary.setStatusDesc(statusDesc);
			temporary.setDistrctName(distrctName);
			temporary.setDisplayRefId(displayRefId);
			temporary.setTalukName(talukName);
			temporary.setCompanyName(companyName);
			temporary.setAppTypeDesc(appTypeDesc);
			temporary.setStateName(stateName);
			temporary.setLicenseCategoryName(licenseCategoryName);
			temporary.setAppSubmitionDate(appSubmitionDate);
			temporary.setLastUpdatedOn(lastUpdatedOn);
			temporary.setFboId(fboId);
			temporary.setReferId(refId);
			temporary.setStatePremesis(statePremesis);
			temporary.setDistrictPremesis(districtPremesis);
			temporary.setTalikPremesis(talikPremesis);
			temporary.setAppType(appType);
			temporary.setPincodeId(pincode);
		...
		JSONObject commonResponse = CommonResponseStructure.commonResponseFssai(temporary);
