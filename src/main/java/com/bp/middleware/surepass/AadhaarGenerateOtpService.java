...
public class AadhaarGenerateOtpService {
	...
	public ResponseStructure aadhaarGenerateOtp(RequestModel model, HttpServletRequest servletRequest) {
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return surepassAadharGenerateOtp(model, userModel, verificationModel, vendorModel,
	...
	private ResponseStructure surepassAadharGenerateOtp(RequestModel model, EntityModel userModel,
