...
import com.bp.middleware.duplicateverificationresponse.DirectorMobileReplica;
...
@Service
public class DirectorMobile {

	private final String VERIFICATION_TYPE = AppConstants.DIRECTOR_MOBILE;
	...
	@Autowired
	private DirectorMobileReplica directorMobileReplica;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;
	
	
	public ResponseStructure directorMobile(RequestModel model, HttpServletRequest servletRequest) {
		...
		try {
			...
			if (userModel != null  && userModel.isAccountStatus()) {
				...
				VendorVerificationModel verificationModel = verificationRepository
						.findByVerificationDocument(AppConstants.DIRECTOR_MOBILE);
			...
			if(!verificationModel.isStatus() || !vendorModel.isStatus()) {
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return directorMobileReplica.directorMobileResponse(model, userModel,verificationModel);
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return surepassDirectorMobile(model, userModel, verificationModel, vendorModel,
	...
	private ResponseStructure surepassDirectorMobile(RequestModel model, EntityModel userModel,
		...
		if (status) {
			...
			String dinNumber = internalData.getString("din_number");
			String phoneNumber = internalData.getString("phone_number");
		...
		JSONObject commonResponse = CommonResponseStructure.commonResponseDirectorMobile(temporary);
