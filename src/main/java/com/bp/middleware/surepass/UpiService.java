...
import com.bp.middleware.duplicateverificationresponse.UpiReplica;
...
public class UpiService {
	...
	private final String VERIFICATION_TYPE = AppConstants.UPI;
	...
	@Autowired
	private UpiReplica upiReplica;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;

	public ResponseStructure unifiedPaymentsInterface(RequestModel model, HttpServletRequest servletRequest) {
		...
		try {
			...
			if (userModel != null  && userModel.isAccountStatus()) {
				...
				VendorVerificationModel verificationModel = verificationRepository
						.findByVerificationDocument(AppConstants.UPI);
			...
			if(!verificationModel.isStatus() || !vendorModel.isStatus()) {
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return upiReplica.upiResponse(model, userModel,verificationModel);
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return surepassUpi(model, userModel, verificationModel, vendorModel, merchantPriceModel,
	...
	private ResponseStructure surepassUpi(RequestModel model, EntityModel userModel,
		...
		if (status) {
			...
			String upiId = internalData.getString("upi_id");
		...
		JSONObject commonResponse = CommonResponseStructure.commonResponseUpi(temporary);
