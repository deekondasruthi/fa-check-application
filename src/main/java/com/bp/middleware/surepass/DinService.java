...
public class DinService {
	...
	public ResponseStructure directorIdentifyNumber(RequestModel model, HttpServletRequest servletRequest) {
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return surepassDin(model, userModel, verificationModel, vendorModel, merchantPriceModel,
	...
	private ResponseStructure surepassDin(RequestModel model, EntityModel userModel,
		...
		if (status) {
			...
			String address = internalData.getString("present_address");
			String dinNumber = internalData.getString("din_number");
			String nationality = internalData.getString("nationality");
			String fatherName = internalData.getString("father_name");
			String dob = internalData.getString("dob");
			String email = internalData.getString("email");
			String dinStatus = internalData.getString("status");
