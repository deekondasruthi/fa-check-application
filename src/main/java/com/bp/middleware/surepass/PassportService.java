...
public class PassportService {
	...
	public ResponseStructure passport(RequestModel model, HttpServletRequest servletRequest) {
			...
			if (userModel != null  && userModel.isAccountStatus()) {
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return passportReplica.passportDuplicateResponse(model, userModel,verificationModel);
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return surepassPassport(model, userModel, verificationModel, vendorModel, merchantPriceModel,
	...
	private ResponseStructure surepassPassport(RequestModel model, EntityModel userModel,
