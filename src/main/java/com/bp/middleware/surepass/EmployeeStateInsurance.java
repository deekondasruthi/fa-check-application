...
import com.bp.middleware.duplicateverificationresponse.EmployeeStateInsuranceReplica;
...
@Service
public class EmployeeStateInsurance {

	
	private final String VERIFICATION_TYPE = AppConstants.ESIC;
	...
	@Autowired
	private EmployeeStateInsuranceReplica employeeStateInsuranceReplica;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;
	
	
	public ResponseStructure employeeStateInsuranceVerification(RequestModel model, HttpServletRequest servletRequest) {
			...
			if(!verificationModel.isStatus() || !vendorModel.isStatus()) {
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return employeeStateInsuranceReplica.employeeStateInsuranceResponse(model, userModel,verificationModel);
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return surepassEsic(model, userModel, verificationModel, vendorModel,
	...
	private ResponseStructure surepassEsic(RequestModel model, EntityModel userModel,
		...
		JSONObject commonResponse = CommonResponseStructure.commonResponseEsic(temporary);
