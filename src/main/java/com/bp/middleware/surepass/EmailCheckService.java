...
import com.bp.middleware.duplicateverificationresponse.EmailCheckReplica;
...
public class EmailCheckService {
	...
	private final String VERIFICATION_TYPE = AppConstants.EMAIL_VERIFY;
	...
	@Autowired
	private EmailCheckReplica emailCheckReplica;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;

	public ResponseStructure emailCheck(RequestModel model, HttpServletRequest servletRequest) {
			...
			if(!verificationModel.isStatus() || !vendorModel.isStatus()) {
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return emailCheckReplica.emailCheckResponse(model, userModel,verificationModel);
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return surepassEmailCheck(model, userModel, verificationModel, vendorModel, merchantPriceModel,
	...
	private ResponseStructure surepassEmailCheck(RequestModel model, EntityModel userModel,
		...
		if (status) {
			...
			String name = internalData.getString("username");
			String email = internalData.getString("email");
			String domain = internalData.getString("domain");
			String emailStatus = internalData.getString("status");
			boolean acceptsMail = internalData.getBoolean("accepts_mail");
			boolean isCatchAll = internalData.getBoolean("is_catch_all");
			boolean valid = internalData.getBoolean("valid");
			boolean validSyntax = internalData.getBoolean("valid_syntax");
			boolean smtpConnected = internalData.getBoolean("smtp_connected");
			boolean isTemporary = internalData.getBoolean("is_temporary");
			boolean disabled = internalData.getBoolean("disabled");
			boolean results = internalData.getBoolean("results");
			
			JSONArray mxRecords = internalData.getJSONArray("mx_records");
			
//			String domainAge = internalData.getString("domain_age");
//			String personAge = internalData.getString("person_match");
//			String domainRegister = internalData.getString("domain_registrar");
//			String organiztionMatch = internalData.getString("organization_mactch");
//			String organization = internalData.getString("organization");
			...
			temporary.setDomain(domain);
			temporary.setEmailStatus(emailStatus);
			temporary.setAcceptsMail(acceptsMail);
			temporary.setCatchAll(isCatchAll);
			temporary.setValid(valid);
			temporary.setValidSyntax(validSyntax);
			temporary.setSmtpConnected(smtpConnected);
			temporary.setTemporary(isTemporary);
			temporary.setDisabled(disabled);
			temporary.setResults(results);
			temporary.setMxRecords(mxRecords);
		...
		JSONObject commonResponse = CommonResponseStructure.commonResponseEmail(temporary);
