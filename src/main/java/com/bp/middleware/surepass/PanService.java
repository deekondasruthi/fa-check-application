...
public class PanService {
	...
	public ResponseStructure pan(RequestModel model, HttpServletRequest servletRequest) {
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return surepassPan(model, userModel, verificationModel, vendorModel, merchantPriceModel,
	...
	private ResponseStructure surepassPan(RequestModel model, EntityModel userModel,
		...
		if (status) {
			...
			String category = internalData.getString("category");
