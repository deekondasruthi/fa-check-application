...
import com.bp.middleware.duplicateverificationresponse.MobileToUanReplica;
...
@Service
public class MobileToUan {

	
	private final String VERIFICATION_TYPE = AppConstants.MOBILE_TO_UAN;
	...
	@Autowired
	private MobileToUanReplica mobileToUanReplica;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;

	public ResponseStructure mobileToUan(RequestModel model, HttpServletRequest servletRequest) {
		...
		try {
			...
			if (userModel != null  && userModel.isAccountStatus()) {
				...
				VendorVerificationModel verificationModel = verificationRepository
						.findByVerificationDocument(AppConstants.MOBILE_TO_UAN);
			...
			if(!verificationModel.isStatus() || !vendorModel.isStatus()) {
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey) && userModel.getNoRestriction() == 0) {
						...
						return mobileToUanReplica.mobileToUanResponse(model, userModel, verificationModel);
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return surepassMobileToUan(model, userModel, verificationModel, vendorModel,
	...
	private ResponseStructure surepassMobileToUan(RequestModel model, EntityModel userModel,
		...
		if (status) {
			...
			temporary.setMobileNumber(source);
		...
		JSONObject commonResponse = CommonResponseStructure.commonResponseMobileToUan(temporary);
