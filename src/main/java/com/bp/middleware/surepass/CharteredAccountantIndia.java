...
import com.bp.middleware.duplicateverificationresponse.CharteredAccountantIndiaReplica;
...
@Service
public class CharteredAccountantIndia {

	
	private final String VERIFICATION_TYPE = AppConstants.ICAI;
	...
	@Autowired
	private CharteredAccountantIndiaReplica charteredAccountantIndiaReplica;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;
	
	
	public ResponseStructure charteredAccountant(RequestModel model, HttpServletRequest servletRequest) {
		...
		try {
			...
			if (userModel != null && userModel.isAccountStatus()) {
				...
				VendorVerificationModel verificationModel = verificationRepository
						.findByVerificationDocument(AppConstants.ICAI);
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return charteredAccountantIndiaReplica.charteredAccountantIndiaResponse(model, userModel,verificationModel);
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return surepassIcai(model, userModel, verificationModel, vendorModel,
	...
	private ResponseStructure surepassIcai(RequestModel model, EntityModel userModel,
		...
		obj.put("membership_number", source);
		...
		if (status) {
			...
			String membershipNumber = internalData.getString("membership_number");
			
			JSONObject details = internalData.getJSONObject("details");
			
			String memberName = details.getString("member_name");
			String gender = details.getString("gender");
			String address = details.getString("professional_address");


			request.setReferenceId(referenceId);
			request.setStatus("success");
			request.setClientId(clientId);
			request.setFullName(memberName);
			request.setAttempt(0);

			response.setClientId(clientId);
			response.setName(memberName);
			response.setFullName(memberName);
			...
			temporary.setMembershipNumber(membershipNumber);
			temporary.setDetails(details);
		...
		JSONObject commonResponse = CommonResponseStructure.commonResponseIcai(temporary);
