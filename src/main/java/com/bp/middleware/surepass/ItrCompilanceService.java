...
import com.bp.middleware.duplicateverificationresponse.ItrCompilanceReplica;
...
@Service
public class ItrCompilanceService {

	
	private final String VERIFICATION_TYPE = AppConstants.ITR_COMPILANCE;
	...
	@Autowired
	private ItrCompilanceReplica itrCompilanceReplica;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;

	public ResponseStructure itrCompilanceCheck(RequestModel model, HttpServletRequest servletRequest) {
		...
		try {
			...
			if (userModel != null  && userModel.isAccountStatus()) {
				...
				VendorVerificationModel verificationModel = verificationRepository
						.findByVerificationDocument(AppConstants.ITR_COMPILANCE);
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey) && userModel.getNoRestriction() == 0) {
						...
						return itrCompilanceReplica.itrCompilanceResponse(model, userModel, verificationModel);
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return surepassItrCompilanceCheck(model, userModel, verificationModel, vendorModel,
	...
	private ResponseStructure surepassItrCompilanceCheck(RequestModel model, EntityModel userModel,
		...
		if (status) {
			...
			String panAllotmentDate = internalData.getString("pan_allotment_date");
			String maskedName = internalData.getString("masked_name");
			String panAadhaarLinked = internalData.getString("pan_aadhaar_linked");
			String specifiedPersonUnder206 = internalData.getString("specified_person_under_206");
			String panStatus = internalData.getString("pan_status");
			boolean validPan = internalData.getBoolean("valid_pan");
			boolean complaint = internalData.getBoolean("compliant");
			...
			temporary.setPanAllotmentDate(panAllotmentDate);
			temporary.setMaskedName(maskedName);
			temporary.setPanAadhaarLinked(panAadhaarLinked);
			temporary.setSpecifiedPersonUnder206(specifiedPersonUnder206);
			temporary.setPanStatus(panStatus);
			temporary.setValidPan(validPan);
			temporary.setComplaint(complaint);
		...
		JSONObject commonResponse = CommonResponseStructure.commonResponseItrCompilanceCheck(temporary);
