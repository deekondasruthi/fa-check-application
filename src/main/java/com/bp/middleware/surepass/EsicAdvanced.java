...
import com.bp.middleware.duplicateverificationresponse.EsicAdvancedReplica;
...
@Service
public class EsicAdvanced {

	
	private final String VERIFICATION_TYPE = AppConstants.ESIC_ADVANCED;
	...
	@Autowired
	private EsicAdvancedReplica esicAdvancedReplica;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;
	
	
	public ResponseStructure esicAdvancedVerification(RequestModel model, HttpServletRequest servletRequest) {
		...
		try {
			...
			if (userModel != null  && userModel.isAccountStatus()) {
				...
				VendorVerificationModel verificationModel = verificationRepository
						.findByVerificationDocument(AppConstants.ESIC_ADVANCED);
			...
			if(!verificationModel.isStatus() || !vendorModel.isStatus()) {
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return esicAdvancedReplica.EsicAdvancedResponse(model, userModel,verificationModel);
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return surepassEsicAdvanced(model, userModel, verificationModel, vendorModel,
	...
	private ResponseStructure surepassEsicAdvanced(RequestModel model, EntityModel userModel,
		...
		if (status) {
			...
			String idNumber = internalData.getString("id_number");
			String name = internalData.getString("name");
			String mobileNumber = internalData.getString("mobile_number");
		...
		JSONObject commonResponse = CommonResponseStructure.commonResponseEsicAdvanced(temporary);
