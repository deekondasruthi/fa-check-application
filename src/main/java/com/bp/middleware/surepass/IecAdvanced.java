...
import com.bp.middleware.duplicateverificationresponse.IecAdvancedReplica;
...
@Service
public class IecAdvanced {

	
	private final String VERIFICATION_TYPE = AppConstants.IEC_ADVANCED;
	...
	@Autowired
	private IecAdvancedReplica iecAdvancedReplica;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;

	public ResponseStructure iecAdvanced(RequestModel model, HttpServletRequest servletRequest) {
			...
			if (userModel != null  && userModel.isAccountStatus()) {
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey) && userModel.getNoRestriction() == 0) {
						...
						return iecAdvancedReplica.IecAdvancedResponse(model, userModel, verificationModel);
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return surepassIecAdvanced(model, userModel, verificationModel, vendorModel,
	...
	private ResponseStructure surepassIecAdvanced(RequestModel model, EntityModel userModel,
		...
		String source = userJson.getString("source");
		String firm = userJson.getString("firm_name");
		...
		obj.put("iec_number", source);
		obj.put("firm_name", firm);
		...
		if (status) {
			...
			String iecNumber = internalData.getString("iec_number");
			String firmName = internalData.getString("firm_name");
			String panNumber = internalData.getString("pan_number");
			String dob = internalData.getString("dob");
			String iecIssuanceDate = internalData.getString("iec_issuance_date");
			String iecStatus = internalData.getString("iec_status");
			String delStatus = internalData.getString("del_status");
			String iecCancelledDate = internalData.getString("iec_cancelled_date");
			String iecSuspendedDate = internalData.getString("iec_suspended_date");
			String fileNumber = internalData.getString("file_number");
			String fileDate = internalData.getString("file_date");
			String dgftraOffice = internalData.getString("dgft_ra_office");
			String natureOfConcern = internalData.getString("nature_of_concern");
			String categoryOfExporters = internalData.getString("category_of_exporters");
			String address = internalData.getString("address");
			String firmMobileNo = internalData.getString("firm_mobile_no");
			String firmEmailId = internalData.getString("firm_email_id");

			JSONArray branchDetails = internalData.getJSONArray("branch_details");
			JSONArray remcDetails = internalData.getJSONArray("rcmc_details");
			JSONArray directorDetails = internalData.getJSONArray("director_details");
			...
			temporary.setIecNumber(iecNumber);
			temporary.setFirmName(firmName);
			temporary.setPanNumber(panNumber);
			temporary.setDob(dob);
			temporary.setIecIssuanceDate(iecIssuanceDate);
			temporary.setIecStatus(iecStatus);
			temporary.setDelStatus(delStatus);
			temporary.setIecCancelledDate(iecCancelledDate);
			temporary.setIecSuspendedDate(iecSuspendedDate);
			temporary.setFileNumber(fileNumber);
			temporary.setFileDate(fileDate);
			temporary.setDgftraOffice(dgftraOffice);
			temporary.setNatureOfConcern(natureOfConcern);
			temporary.setCategoryOfExporters(categoryOfExporters);
			temporary.setAddress(address);
			temporary.setFirmMobileNo(firmMobileNo);
			temporary.setFirmEmailId(firmEmailId);
			temporary.setBranchDetails(branchDetails);
			temporary.setRemcDetails(remcDetails);
			temporary.setDirectorDetails(directorDetails);
		...
		JSONObject commonResponse = CommonResponseStructure.commonResponseIecAdvanced(temporary);
