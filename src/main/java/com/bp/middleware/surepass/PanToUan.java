...
import com.bp.middleware.duplicateverificationresponse.PanToUanReplica;
...
@Service
public class PanToUan {

	
	private final String VERIFICATION_TYPE = AppConstants.PAN_TO_UAN;
	...
	@Autowired
	private PanToUanReplica panToUanReplica;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;
	
	
	public ResponseStructure panToUan(RequestModel model, HttpServletRequest servletRequest) {
		...
		try {
			...
			if (userModel != null  && userModel.isAccountStatus()) {
				...
				VendorVerificationModel verificationModel = verificationRepository
						.findByVerificationDocument(AppConstants.PAN_TO_UAN);
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return panToUanReplica.PanToUanResponse(model, userModel,verificationModel);
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return surepassPanToUan(model, userModel, verificationModel, vendorModel,
	...
	private ResponseStructure surepassPanToUan(RequestModel model, EntityModel userModel,
		...
		if (status) {
			...
			String pfUan = internalData.getString("uan_number");
		...
		JSONObject commonResponse = CommonResponseStructure.commonResponsePanToUan(temporary);
