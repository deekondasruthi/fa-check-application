...
public class VoterIdService {
			...
			if (userModel != null  && userModel.isAccountStatus()) {
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return voterIdReplica.voterIdDuplicateResponse(model, userModel,verificationModel);
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return surepassVoterId(model, userModel, verificationModel, vendorModel, merchantPriceModel,
	...
	private ResponseStructure surepassVoterId(RequestModel model, EntityModel userModel,
		...
		if (status) {
			...
			String psLatLong = internalData.getString("ps_lat_long");
			...
			String polling_station = internalData.getString("polling_station");
