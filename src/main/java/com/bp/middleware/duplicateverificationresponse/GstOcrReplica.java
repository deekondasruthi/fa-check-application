...
@Component
public class GstOcrReplica {

	@Autowired
	private DuplicateUtils duplicateUtils;

	public ResponseStructure gstOcrResponse(MultipartFile model, EntityModel userModel,
		...
		reqModel.setSource("GST OCR Image");
		...
		if (true) {
			...
			arrayObject.put("document_type", "gst_reg");

			JSONObject gstin = new JSONObject();

			gstin.put("value", "33AAFCB2854F2ZY");
			gstin.put("confidence", 96);

			arrayObject.put("gstin", gstin);

			arrayObject.put("standard_document", true);
