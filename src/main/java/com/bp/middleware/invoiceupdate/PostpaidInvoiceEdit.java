...
@Component
public class PostpaidInvoiceEdit {
	...
	public String updatePostpaidCgstInvoice(TransactionDto transactionDto, PostpaidPayment postpaid, RequestModel model,
			Path con) throws Exception {

		String generatedIn = "";

		String invoiceNumber = postpaid.getInvoiceNumber();
		String generatedDate = model.getGeneratedDate().toString();
		String payDate = model.getPaymentDate();
		String startDate = model.getFromDate().toString();
		String endDate = model.getToDate().toString();
		String usedServices = model.getUsedServices();

		int totalHits = model.getTotalHits();
		double price = model.getIdPrice();

		double cgst = model.getIdPrice()*9/100;
		double sgst = model.getIdPrice()*9/100;

		double total = fu.twoDecimelDouble(price+cgst+sgst);
...
//				+ "</td>\r\n"
		...
		if (postpaid.getInvoiceTwo() == null || postpaid.getInvoiceTwo().equalsIgnoreCase("Deleted")) {
			...
			postpaid.setInvoiceTwo(generatedString);

		} else if (postpaid.getInvoiceThree() == null || postpaid.getInvoiceThree().equalsIgnoreCase("Deleted")) {
			...
			postpaid.setInvoiceThree(generatedString);
		} else {
			generatedIn = "C";
		}
	...
	public String updatePostpaidIgstInvoice(TransactionDto transactionDto, PostpaidPayment postpaid,
		...
		double igst = model.getIdPrice()*18/100;

		double total = fu.twoDecimelDouble(model.getTotalAmount()+igst);
