...
public class PanVerification {
	...
	public ResponseStructure panVerification(RequestModel model, HttpServletRequest servletRequest) {
			...
			if (userModel != null && userModel.isAccountStatus()) {
				...
				if (!merchantPriceList.isEmpty() && !vendorPriceList.isEmpty()  &&  accepted) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey) && userModel.getNoRestriction() == 0) {
						...
						return panReplica.panDuplicateResponse(model, userModel, vendorVerifyModel);
					...
					if (balanceCheck.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return panVerificationSmartRoute(userJson, model, userModel, vendorVerifyModel);
						}
					}
	...
	public ResponseStructure panVerificationSmartRoute(JSONObject userJson, RequestModel model, EntityModel userModel,
...
//		int priority = attempt + 1;
	...
	public ResponseStructure signDeskPanVerification(JSONObject userJson, RequestModel model, EntityModel userModel,
		...
		System.err.println("SIGN DESK");
		System.err.println(vendorPrice.getApiKey());
		System.err.println(vendorPrice.getApplicationId());
		...
		if (signDeskStatus.equals("success")) {
			...
			if (validPan) {
				...
				String name = validJson.optString("full_name", "");
				String panNumber = validJson.optString("pan_number", "");
				String category = validJson.optString("category", "");
	...
	private ResponseStructure sprintVerifyPanVerification(JSONObject userJson, RequestModel model,
	...
	private ResponseStructure surepassPanVerification(JSONObject userJson, RequestModel model, EntityModel userModel,
			VendorVerificationModel vendorVerifyModel, VendorModel vendorModel, MerchantPriceModel merchantPriceModel,
			VendorPriceModel vendorPrice) throws Exception {
