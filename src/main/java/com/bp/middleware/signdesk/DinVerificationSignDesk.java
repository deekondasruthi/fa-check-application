...
@Service
public class DinVerificationSignDesk {
	...
	public ResponseStructure dinVerificationService(RequestModel model ,HttpServletRequest servletRequest) {
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return signDeskDinVerification(userJson, model, userModel, verificationModel, vendorModel,
	...
	public ResponseStructure signDeskDinVerification(JSONObject userJson, RequestModel model, EntityModel userModel,
		...
		if (status.equals("success")) {
			...
			if (validDin) {
				...
				String dinNumber = validatedJson.getString("din_number");
				String fatherName = validatedJson.getString("father_name");
				String fullName = validatedJson.getString("full_name");

				String dob = validatedJson.getString("dob");
				String nationality = validatedJson.getString("nationality");
				String address = validatedJson.getString("present_address");
