...
@Service
public class RcVerificationSignDesk {
	...
	public ResponseStructure rcVerificationService(RequestModel model ,HttpServletRequest servletRequest) {
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (balance.getFlag() == 1) {
						...
						if (sourceResponse.getResponseId() > 0) {
							...
							return signDeskRcVerification(userJson, model, userModel, verificationModel, vendorModel,
	...
	public ResponseStructure signDeskRcVerification(JSONObject userJson, RequestModel model, EntityModel userModel,
		...
		//String commonResponse = CommonResponseStructure.commonResponseRc(response, userModel);
