...
import com.bp.middleware.duplicateverificationresponse.PassportOcrReplica;
...
public class PassportOcrService {
	...
	@Autowired
	private PassportOcrReplica passportOcrReplica;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;

	public ResponseStructure passportOcr(MultipartFile file, HttpServletRequest servletRequest) {
		...
		try {
			...
			if (userModel != null && userModel.isAccountStatus()) {
				...
				VendorVerificationModel vendorVerifyModel = vendorVerificationRepository
						.findByVerificationDocument(AppConstants.PASSPORT_IMAGE);
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return passportOcrReplica.PassportOcrResponse(file, userModel,vendorVerifyModel);
					...
					if (balanceCheck.getFlag() == 1) {
						...
						return passportOcrVerification(userModel, vendorModel, vendorVerifyModel, merchantPriceModel,
								file,model);
	...
	private ResponseStructure passportOcrVerification(EntityModel entityModel, VendorModel vendorModel,
...
//		String surepassResponse = AppConstants.OCR_AADHAAR;
		...
		if (status) {
			...
			JSONObject countryCodeDetails = indexZeroFromList.getJSONObject("country_code");
			JSONObject dobDetails = indexZeroFromList.getJSONObject("dob");
			JSONObject doeDetails = indexZeroFromList.getJSONObject("doe");
			JSONObject doiDetails = indexZeroFromList.getJSONObject("doi");
			JSONObject genderDetails = indexZeroFromList.getJSONObject("gender");
			JSONObject givenNameDetails = indexZeroFromList.getJSONObject("given_name");
			JSONObject nationalityDetails = indexZeroFromList.getJSONObject("nationality");
			JSONObject passportNumberDetails = indexZeroFromList.getJSONObject("passport_num");
			JSONObject birthPlaceDetails = indexZeroFromList.getJSONObject("place_of_birth");
			JSONObject issuePlaceDetails = indexZeroFromList.getJSONObject("place_of_issue");
			JSONObject surnameDetails = indexZeroFromList.getJSONObject("surname");
			JSONObject mrzLine1Details = indexZeroFromList.getJSONObject("mrz_line_1");
			JSONObject mrzLine2Details = indexZeroFromList.getJSONObject("mrz_line_2");
			JSONObject typeOfPassportDetails = indexZeroFromList.getJSONObject("type_of_passport");

			String countryCode = countryCodeDetails.getString("value");
			String dob = dobDetails.getString("value");
			String doe = doeDetails.getString("value");
			String doi = doiDetails.getString("value");
			String gender = genderDetails.getString("value");
			String name = givenNameDetails.getString("value");
			String nationality = nationalityDetails.getString("value");
			String passportNumber = passportNumberDetails.getString("value");
			String birthplace = birthPlaceDetails.getString("value");
			String issuePlace = issuePlaceDetails.getString("value");
			String surname = surnameDetails.getString("value");
			String mrzLine1 = mrzLine1Details.getString("value");
			String mrzLine2 = mrzLine2Details.getString("value");
			String passportType = typeOfPassportDetails.getString("value");

			request.setSource(passportNumber + " OCR");
			request.setFullName(name);
			request.setDob(dob);
			request.setClientId(clientId);
			request.setStatus("success");

			response.setSource(passportNumber + " OCR");
