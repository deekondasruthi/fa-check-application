...
import com.bp.middleware.duplicateverificationresponse.DocumentDetectOcrReplica;
...
@Service
public class DocumentDetectOcrService {

	
	private final String VERIFICATION_TYPE = AppConstants.DOCUMENTDETECT_OCR;
	...
	@Autowired
	private DocumentDetectOcrReplica documentDetectReplica;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;

	public ResponseStructure documentDetectOcr(MultipartFile file, HttpServletRequest servletRequest) {
		...
		try {
			...
			if (userModel != null && userModel.isAccountStatus()) {
				...
				VendorVerificationModel vendorVerifyModel = vendorVerificationRepository
						.findByVerificationDocument(AppConstants.DOCUMENTDETECT_OCR);
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return documentDetectReplica.documentDetectResponse(file, userModel,vendorVerifyModel);
					...
					if (balanceCheck.getFlag() == 1) {
						...
						return documentDetectOcrVerification(userModel, vendorModel, vendorVerifyModel, merchantPriceModel,
								file,model);
					}
	...
	private ResponseStructure documentDetectOcrVerification(EntityModel entityModel, VendorModel vendorModel,
...
//			String clientId = data.getString("client_id");
			...
			JSONObject indexZeroFromList = documents.getJSONObject(0);

//			String documentType = indexZeroFromList.getString("document_type");

			String docType = indexZeroFromList.getString("type");

			request.setSource(docType + " OCR");
