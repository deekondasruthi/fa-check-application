...
import com.bp.middleware.duplicateverificationresponse.GstOcrReplica;
...
public class GstOcrService {
	...
	@Autowired
	private GstOcrReplica gstOcrResponse;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;

	public ResponseStructure gstOcr( MultipartFile file, HttpServletRequest servletRequest) {
			...
			if (userModel != null && userModel.isAccountStatus()) {
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return gstOcrResponse.gstOcrResponse(file, userModel,vendorVerifyModel);
					...
					if (balanceCheck.getFlag() == 1) {
						...
						return gstOcrVerification(userModel, vendorModel, vendorVerifyModel, merchantPriceModel,
								file,model);
					}
	...
	private ResponseStructure gstOcrVerification(EntityModel entityModel, VendorModel vendorModel,
