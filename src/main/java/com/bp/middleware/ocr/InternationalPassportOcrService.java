...
import com.bp.middleware.duplicateverificationresponse.InternationalPassportOcrReplica;
...
@Service
public class InternationalPassportOcrService {

	
	private final String VERIFICATION_TYPE = AppConstants.INTL_PASSPORT;
	...
	@Autowired
	private InternationalPassportOcrReplica internationalPassportOcrReplica;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;

	public ResponseStructure internationalPassportOcr(MultipartFile file, HttpServletRequest servletRequest) {
		...
		try {
			...
			if (userModel != null && userModel.isAccountStatus()) {
				...
				VendorVerificationModel vendorVerifyModel = vendorVerificationRepository
						.findByVerificationDocument(AppConstants.INTL_PASSPORT);
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return internationalPassportOcrReplica.InternationalPassportOcrResponse(file, userModel,vendorVerifyModel);
					...
					if (balanceCheck.getFlag() == 1) {
						...
						return internationalPassportOcrVerification(userModel, vendorModel, vendorVerifyModel, merchantPriceModel,
								file,model);
	...
	private ResponseStructure internationalPassportOcrVerification(EntityModel entityModel, VendorModel vendorModel,
...
//		String surepassResponse = AppConstants.OCR_AADHAAR;
		...
		if (status) {
			...
			String type = indexZeroFromList.getString("type");

			JSONObject details = indexZeroFromList.getJSONObject("details");

			String birthDate = details.getString("birth_date");
			String birthDateHash = details.getString("birth_date_hash");
			String countryCode = details.getString("country_code");
			String country = details.getString("country");
			String documentNumber = details.getString("document_number");
			String documentNumberHash = details.getString("document_number_hash");
			
			String documentType = details.getString("document_type");
			String expiryDate = details.getString("expiry_date");
			String expiryDateHash = details.getString("expiry_date_hash");
			String finalHash = details.getString("final_hash");
			String name = details.getString("name");
			String nationalityCode = details.getString("nationality_code");
			
			String nationality = details.getString("nationality");
			String optionalData = details.getString("optional_data");
			String optionalDataHash = details.getString("optional_data_hash");
			String gender = details.getString("sex");
			String surname = details.getString("surname");
			String mrzLineOne = details.getString("mrz_line_1");
			String mrzLineTwo = details.getString("mrz_line_2");
			
			

			request.setSource(documentNumber + " OCR");
			request.setFullName(name);
			request.setDob(birthDate);
			request.setClientId(clientId);
			request.setStatus("success");

			response.setSource(documentNumber + " OCR");
