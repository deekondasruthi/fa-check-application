...
import com.bp.middleware.duplicateverificationresponse.LicenseOcrReplica;
...
@Service
public class LicenseOcrService {

	
	private final String VERIFICATION_TYPE = AppConstants.DL_OCR;
	...
	@Autowired
	private LicenseOcrReplica  licenseOcrReplica;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;

	public ResponseStructure licenseOcr(MultipartFile front, MultipartFile back, HttpServletRequest servletRequest) {
		...
		try {
			...
			if (userModel != null && userModel.isAccountStatus()) {
				...
				int fileFormatCheckFlag = smartRouteUtils.FileFormatCheck(front,userModel);
				int fileFormatCheckFlag2 = smartRouteUtils.FileFormatCheck(back,userModel);
				

				VendorVerificationModel vendorVerifyModel = vendorVerificationRepository
						.findByVerificationDocument(AppConstants.DL_OCR);
				...
				}else if(fileFormatCheckFlag2 == 2 || fileFormatCheckFlag2 == 3 || fileFormatCheckFlag2 == 4) {
					
					return smartRouteUtils.fileFormatFailed(fileFormatCheckFlag2,userModel,vendorVerifyModel);
				}
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return licenseOcrReplica.licenseOcrResponse(front,back, userModel,vendorVerifyModel);
					...
					if (balanceCheck.getFlag() == 1) {
						...
						return licenseOcrVerification(userModel, vendorModel, vendorVerifyModel, merchantPriceModel,
								front,back,model);
	...
	private ResponseStructure licenseOcrVerification(EntityModel entityModel, VendorModel vendorModel,
			VendorVerificationModel vendorVerifyModel, MerchantPriceModel merchantPriceModel, MultipartFile front, MultipartFile back,RequestModel model)
			throws Exception {
		...
		String surepassResponse = ocrMechanisms.ocrMechanismsWithTwoMultiparts(entityModel,merchantPriceModel,front,back);
...
//		String surepassResponse = AppConstants.OCR_AADHAAR;
		...
		request.setRequestBy(entityModel.getName());
		...
		response.setRequestBy(entityModel.getName());
...
//			String documentType = data.getString("document_type");

			JSONObject licenseDetails = data.getJSONObject("license_number");
			JSONObject dobDetails = data.getJSONObject("dob");

			String licenseNumber = licenseDetails.getString("value");
			String dob = dobDetails.getString("value");

			request.setSource(licenseNumber + " OCR");
			request.setDob(dob);
			request.setClientId(clientId);
			request.setStatus("success");

			response.setSource(licenseNumber + " OCR");
