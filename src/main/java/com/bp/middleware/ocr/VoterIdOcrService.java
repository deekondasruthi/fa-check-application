...
import com.bp.middleware.duplicateverificationresponse.VoterIdOcrReplica;
...
@Service
public class VoterIdOcrService {

	
	private final String VERIFICATION_TYPE = AppConstants.VOTERID_OCR;
	...
	@Autowired
	private VoterIdOcrReplica voterIdOcrResponse;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;

	public ResponseStructure voterIdOcr(MultipartFile file, HttpServletRequest servletRequest) {
		...
		try {
			...
			if (userModel != null && userModel.isAccountStatus()) {
				...
				VendorVerificationModel vendorVerifyModel = vendorVerificationRepository
						.findByVerificationDocument(AppConstants.VOTERID_OCR);
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return voterIdOcrResponse.VoterIdOcrResponse(file, userModel,vendorVerifyModel);
					...
					if (balanceCheck.getFlag() == 1) {
						...
						return voterIdOcrVerification(userModel, vendorModel, vendorVerifyModel, merchantPriceModel,
								file,model);
					}
	...
	private ResponseStructure voterIdOcrVerification(EntityModel entityModel, VendorModel vendorModel,
	...
	//	String surepassResponse = AppConstants.OCR_VOTERID;
...
//			String documentType = indexZeroFromList.getString("document_type");
			...
			JSONObject fullNameDetails = indexZeroFromList.getJSONObject("full_name");
			JSONObject ageDetails = indexZeroFromList.getJSONObject("age");
			JSONObject careOfDetails = indexZeroFromList.getJSONObject("care_of");
			JSONObject dobDetails = indexZeroFromList.getJSONObject("dob");
			JSONObject docDetails = indexZeroFromList.getJSONObject("doc");
			JSONObject genderDetails = indexZeroFromList.getJSONObject("gender");
			JSONObject epicNumberDetails = indexZeroFromList.getJSONObject("epic_number");

			String name = fullNameDetails.getString("value");
			String age = ageDetails.getString("value");
			String careOf = careOfDetails.getString("value");
			String dob = dobDetails.getString("value");
			String doc = docDetails.getString("value");
			String gender = genderDetails.getString("value");
			String epicNumber = epicNumberDetails.getString("value");

			request.setSource(epicNumber + " OCR");
			request.setFullName(name);
			request.setDob(dob);
			request.setStatus("success");
			request.setClientId(clientId);

			response.setSource(epicNumber + " OCR");
