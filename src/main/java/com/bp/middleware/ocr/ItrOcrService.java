...
import com.bp.middleware.duplicateverificationresponse.ItrOcrReplica;
...
@Service
public class ItrOcrService {

	
	private final String VERIFICATION_TYPE = AppConstants.ITR_OCR;
	...
	@Autowired
	private ItrOcrReplica itrOcrReplica;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;

	public ResponseStructure itrOcr(MultipartFile file,String pdf, HttpServletRequest servletRequest) {
		...
		try {
			...
			if (userModel != null && userModel.isAccountStatus()) {
				...
				VendorVerificationModel vendorVerifyModel = vendorVerificationRepository
						.findByVerificationDocument(AppConstants.ITR_OCR);
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return itrOcrReplica.itrOcrResponse(file, userModel,vendorVerifyModel);
					...
					if (balanceCheck.getFlag() == 1) {
						...
						return itrOcrVerification(userModel, vendorModel, vendorVerifyModel, merchantPriceModel,
								file,model);
					}
	...
	private ResponseStructure itrOcrVerification(EntityModel entityModel, VendorModel vendorModel,
...
//			String documentType = indexZeroFromList.getString("document_type");
			...
			JSONObject gstIn = indexZeroFromList.getJSONObject("gstin");
			
			String gst = gstIn.getString("value");

			request.setSource(gst + " OCR");
			request.setStatus("success");
			request.setClientId(clientId);

			response.setSource(gst + " OCR");
