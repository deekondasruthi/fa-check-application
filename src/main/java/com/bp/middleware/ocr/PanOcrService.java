...
import com.bp.middleware.duplicateverificationresponse.PanOcrReplica;
...
public class PanOcrService {
	...
	private final String VERIFICATION_TYPE = AppConstants.PAN_IMAGE;
	...
	@Autowired
	private PanOcrReplica panOcrReplica;
	@Autowired
	private GetPublicIpAndLocation ipAndLocation;

	public ResponseStructure panOcr(MultipartFile file, HttpServletRequest servletRequest) {
		...
		try {
			...
			if (userModel != null && userModel.isAccountStatus()) {
				...
				VendorVerificationModel vendorVerifyModel = vendorVerificationRepository
						.findByVerificationDocument(AppConstants.PAN_IMAGE);
				...
				if (merchantPriceModel != null && vendorPriceModel!=null && merchantPriceModel.isAccepted()) {
					...
					if (userModel.getApiSandboxKey().equals(apiKey)  && userModel.getNoRestriction()==0) {
						...
						return panOcrReplica.PanOcrResponse(file, userModel,vendorVerifyModel);
					...
					if (balanceCheck.getFlag() == 1) {
						...
						return panOcrVerification(userModel, vendorModel, vendorVerifyModel, merchantPriceModel,
								file,model);
					}
	...
	private ResponseStructure panOcrVerification(EntityModel entityModel, VendorModel vendorModel,
...
//		String surepassResponse = AppConstants.OCR_PAN;
...
//			String documentType = indexZeroFromList.getString("document_type");
			...
			JSONObject fatherNameDetails = indexZeroFromList.getJSONObject("father_name");
			JSONObject dobDetails = indexZeroFromList.getJSONObject("dob");
			JSONObject panNumberDetails = indexZeroFromList.getJSONObject("pan_number");

			String name = fullNameDetails.getString("value");
			String fatherName = fatherNameDetails.getString("value");
			String dob = dobDetails.getString("value");
			String panNumber = panNumberDetails.getString("value");

			request.setSource(panNumber + " OCR");
			request.setFullName(name);
			request.setDob(dob);
			request.setStatus("success");
			request.setClientId(clientId);

			response.setSource(panNumber + " OCR");
